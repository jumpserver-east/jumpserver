...
没有新增清理工作流。其他默认分支中的工作流也来自上游。

## 保留的依赖

保留 `jms-generic-action-handler.yml`。它存在于上游镜像分支中，并接收这些分支的
push 事件；默认分支的 `build-component-image.yml` 通过 `workflow_run` 监听它。
如果停用它，这条自动镜像构建链会失去 push 触发入口。
...
文件保留在 Git 中，便于追踪上游变更；新增的其他工作流不会被这一明确名单误停用。
