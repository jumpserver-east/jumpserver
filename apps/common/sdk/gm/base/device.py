...
class Device:
    ...
    def close(self):
        if self.__device is None:
            raise GMDeviceError("device not turned on", -1)
        ret = self._driver.SDF_CloseDevice(self.__device)
        if ret != 0:
            raise GMDeviceError("turn off device failed", ret)
        self.__device = None
    ...
    def new_session(self):
        ...
        if ret != 0:
            raise GMDeviceError("create session failed", ret)
    ...
    def __open_device(self):
        ...
        if ret != 0:
            raise GMDeviceError("open {} device failed".format(self.name), ret)
